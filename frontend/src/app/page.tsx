...
// Seed fallback data
import { SEED_ASSETS, SEED_REQUESTS } from "../lib/store";
...
export default function Home() {
  ...
  // Data state — pre-populated with seed data so the app works offline
  const [assets, setAssets]               = useState<Asset[]>(SEED_ASSETS as Asset[]);
  const [requests, setRequests]           = useState<ResourceRequest[]>(SEED_REQUESTS as ResourceRequest[]);
  ...
  const loadMatching = useCallback(async () => {
    ...
    try {
      ...
      // Backend offline — use seed requests already in state, compute local matches
      const localMatches = assets
        .filter(a => a.lifecycleStatus === "AVAILABLE" && ["EXCELLENT", "GOOD"].includes(a.condition))
        .slice(0, 3)
        .map(a => ({
          assetId: a.id,
          compatibilityScore: Math.round((0.72 + (a.specs?.length ?? 0) * 0.05) * 100) / 100,
          reasons: ["Category matches request.", "Asset is available and in good condition.", `Specs: ${(a.specs ?? []).join(", ") || "N/A"}.`],
          condition: a.condition,
          location: a.location,
        }));
      setMatches(localMatches);
      const msg = err instanceof Error ? err.message : "Resource matching failed.";
      setMatchingError("Backend offline — showing local match estimates.");
      console.info("Matching fallback:", msg);
    } finally {
  ...
  // Initial load — fetch assets on mount; fall back to seed data if backend unavailable
  useEffect(() => {
    async function init() {
      setAssetsLoading(true);
      try {
        ...
        // Backend unreachable — seed data is already loaded, just log
        console.info("Backend unavailable, showing seed data:", err instanceof Error ? err.message : err);
        showToast("Backend offline — showing demo data. Connect MongoDB to enable full features.", "info");
      } finally {
        setAssetsLoading(false);
      }
    }
          ...
          <PageTransition pageKey={activePage}>
            ...
            {activePage === "Asset Registry" && (
              <AssetRegistry
                ...
                loading={assetsLoading}
                onAnalyze={openAnalyze}
                onAdd={(newAsset) => setAssets(prev => [newAsset, ...prev])}
              />
            )}
